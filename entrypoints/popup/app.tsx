...
// Demo mode: the standalone web build (the marketing landing page) sets this
// global *before* this module evaluates — via its `wxt/browser` alias mock,
// which the `import { browser }` above resolves first — so the popup renders
// with mock data outside the extension. Unset in the real extension → URL-driven.
type DemoConfig = { enabled: boolean; locale?: SupportedLocale | null; variant?: string; version?: string };
const DEMO: DemoConfig | undefined = (globalThis as { __STREAM_AUTOPILOT_DEMO__?: DemoConfig }).__STREAM_AUTOPILOT_DEMO__;
const URL_PARAMS = new URLSearchParams(typeof window !== "undefined" ? window.location.search : "");
export const SCREENSHOT_MODE = DEMO?.enabled ?? (URL_PARAMS.get("screenshot") === "store");
// In screenshot mode the capture script drives the rendered language via
// ?locale=<code>; the demo build passes it through DEMO.locale instead.
const SCREENSHOT_LOCALE: SupportedLocale | null = (() => {
  if (!SCREENSHOT_MODE) return null;
  if (DEMO) return DEMO.locale && SUPPORTED_LOCALES.includes(DEMO.locale) ? DEMO.locale : null;
...
// Extension-only; never used by the demo build.
export const PROMO_MODE = !DEMO && URL_PARAMS.get("screenshot") === "promo";
...
const EXTENSION_VERSION = SCREENSHOT_MODE ? (DEMO?.version ?? "1.0.0") : browser.runtime.getManifest().version;
const getMessage = (browser.i18n?.getMessage ?? ((key: string) => key)) as (key: string, substitutions?: string | string[]) => string;
const getUrl = (path: string) => (browser.runtime?.getURL ? browser.runtime.getURL(path as never) : path);
...
const SCREENSHOT_VARIANT_ID = DEMO?.variant ?? URL_PARAMS.get("variant") ?? "twitch-drops";
export const SCREENSHOT_VARIANT = SCREENSHOT_VARIANTS[SCREENSHOT_VARIANT_ID] ?? SCREENSHOT_VARIANTS["twitch-drops"];
...
export function Popup(): React.ReactElement {
  ...
  const locale = effectiveLocale(languageOverride, browser.i18n?.getUILanguage?.() ?? "en");
...
export function StoreScreenshot({ variant, children }: { variant: ScreenshotVariant; children: React.ReactNode }): React.ReactElement {
...
export function PromoTile({ format }: { format: "small" | "marquee" }): React.ReactElement {
