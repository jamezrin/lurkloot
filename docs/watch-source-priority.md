...
1. **Stack on #541.** This implementation depends on its supplemental watch lane. The PR targets `feat/twitch-extension-foundation`; merge #541 and retarget this PR to `develop` before merging it.
...
`pnpm verify` passed all workspace typechecks, 110 extension test files (2,260 tests), 13 CLI test files (199 tests), 95 tooling tests, site tests/build and Chromium/Firefox builds. `pnpm build:cli` and `pnpm release:check` also passed. Independent settings/CLI and engine reviews found no remaining material issues after regression-backed fixes, including snapshot selection preserving Drops progress when preferred Idle yields.
