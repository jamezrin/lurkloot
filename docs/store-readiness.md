...
- **`https://nopixel.streamingtoolsmith.com/*`** — Allows the opted-in NoPixelV provider to initialize its normal viewer session, read daily card-pack watchtime and channel giveaway state, and join an open giveaway. Its backend receives the channel-scoped Twitch Extension viewer authorization, not the user's Twitch session cookie. Giveaway activity is emitted only after the backend confirms membership.
