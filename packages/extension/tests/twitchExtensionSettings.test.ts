...
describe("extension-only provider settings", () => {
  it("defaults all providers and takeovers off without changing tabless defaults", () => {
    expect(mergeSettings(undefined).twitchExtensions).toEqual({ nopixel: { enabled: false }, fortnite: { enabled: false, allowTakeovers: false } });
  ...
  it("merges individual provider toggles without losing other preferences", () => {
    ...
    const patched = applySettingsPatch(original, { twitchExtensions: { nopixel: { enabled: true }, fortnite: { enabled: false } } });
    expect(patched.twitchExtensions).toEqual({ nopixel: { enabled: true }, fortnite: { enabled: false, allowTakeovers: true } });
  });
