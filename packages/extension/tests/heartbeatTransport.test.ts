...
describe("Twitch heartbeat extension transport", () => {
  it("identifies destination-fetch failures without leaking URL details", async () => {
    ...
    expect(error).toEqual(new Error("Twitch Spade destination fetch failed for assets.twitch.tv: Failed to fetch"));
    expect(String(error)).not.toMatch(/settings\.secret|do-not-log/);
  });
  ...
  it("identifies heartbeat POST failures without leaking URL details", async () => {
    ...
    expect(error).toEqual(new Error("Twitch Spade heartbeat POST failed for spade.twitch.tv: Failed to fetch"));
